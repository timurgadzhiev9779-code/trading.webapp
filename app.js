...
if (posValueEl) posValueEl.textContent = `$${portfolio.positions_value.toLocaleString('en-US', {minimumFractionDigits:
2})}`;

const totalValueEl = document.getElementById('total-value');
if (totalValueEl) totalValueEl.textContent = `$${portfolio.total_value.toLocaleString('en-US', {minimumFractionDigits:
2})}`;
...
pnlEl.textContent = `$${portfolio.total_pnl >= 0 ? '+' : ''}${portfolio.total_pnl.toFixed(2)} (${portfolio.total_pnl_pct
>= 0 ? '+' : ''}${portfolio.total_pnl_pct.toFixed(2)}%)`;
}
...
<div class="position-header">
    ...
    <div class="position-pnl ${pnlClass}">
        ${pos.pnl >= 0 ? '📈' : '📉'} $${pos.pnl >= 0 ? '+' : ''}${pos.pnl.toFixed(2)} (${pos.pnl_pct >= 0 ? '+' :
...
<div class="trade-details">
    ...
    <div class="detail-row">
        ...
        <span class="detail-value ${pnlClass}">${trade.profit_pct >= 0 ? '+' : ''}${trade.profit_pct?.toFixed(2) ||
            '0.00'}%</span>
    </div>
    <div class="detail-row">
...
if (diff < 60) return `${diff} сек назад`; if (diff < 3600) return `${Math.floor(diff / 60)} мин назад`; if (diff <
    86400) return `${Math.floor(diff / 3600)} ч назад`; return date.toLocaleDateString('ru-RU'); }
